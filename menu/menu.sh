...
clear
clear && clear && clear
# --- Mulai Skrip Menu (Versi Final Gabungan) ---
clear
...
# --- Info Server & Status (Tetap Dibuat Super Ringkas) ---
cek_status() {
    if [[ $(systemctl is-active "$1") == "active" ]]; then echo -e "${GR}ON${NC}"; else echo -e "\e[31mOFF${NC}"; fi
}
printf " ${WH}%-8s: ${YL}%s${NC}\n" "OS" "$MODEL2"
printf " ${WH}%-8s: ${YL}%s / %s${NC}\n" "IP/Dom" "$MYIP" "$(cat /etc/xray/domain)"
printf " ${WH}%-8s: ${YL}%s%% / %sMB${NC}\n" "CPU/RAM" "$cpu_usage" "$uram"
printf " ${WH}%-8s: ${YL}%s${NC}\n" "Lokasi" "$ISP - $CITY"
printf " ${WH}%-8s: ${WH}[XRAY:${NC}$(cek_status xray)${WH}] [NGX:${NC}$(cek_status nginx)${WH}] [SSH:${NC}$(cek_status sshd)${WH}]\n" "Status"
echo ""

# --- KOTAK INFO CLIENT (DIKEMBALIKAN SESUAI PERMINTAAN) ---
echo -e "            ${GR}╭──────────────────────────────────╮${NC}"
echo -e "            ${GR}│   ${WH}Client: ${YL}$author ${WH}| Version: ${YL}V3.12   ${GR}│${WH}Dev:AcilShop${NC}"
echo -e "            ${GR}╰──────────────────────────────────╯${NC}"
echo ""

# --- Statistik (Dibuat Satu Baris & Ringkas) ---
printf " ${WH}BW Today:${YL}%s ${WH}Total Acc:${YL}%s ${WH}Expire:${YL}$(datediff "$Exp2" "$(date +'%Y-%m-%d')")${NC}\n" "$today_tx" "$(($total_ssh+$vmess+$vless+$trtls+$jumlah_trgo))"
echo ""

# --- GAYA MENU UTAMA (DIKEMBALIKAN SESUAI PERMINTAAN) ---
echo -e "${GR}═══════════════════════════• ${WH}MENU UTAMA${GR} •═════════════════════════════${NC}"
echo ""
echo -e "        ${GR}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
# Bagian 1: Dua Kolom untuk item-item pendek
printf "                       ${GR}[${WH} 1${GR}]${NC} %-15s ${GR}[${WH} 5${GR}]${NC} %-s\n" "SSH & OpenVPN" "Vmess"
printf "                       ${GR}[${WH} 2${GR}]${NC} %-15s ${GR}[${WH} 6${GR}]${NC} %-s\n" "Vless" "Trojan"
printf "                       ${GR}[${WH} 3${GR}]${NC} %-15s ${GR}[${WH} 7${GR}]${NC} %-s\n" "Trojan-Go" "Menu Sistem"
printf "                       ${GR}[${WH} 4${GR}]${NC} %-15s ${GR}[${WH} 8${GR}]${NC} %-s\n" "NoobzVPN" "Update Script"
echo ""

# Bagian 2: Satu Kolom untuk item-item panjang agar tidak berantakan
printf "                       ${GR}[${WH} 9${GR}]${NC} Cek Layanan Aktif\n"
printf "                       ${GR}[${WH}10${GR}]${NC} Restart Layanan\n"
printf "                       ${GR}[${WH}11${GR}]${NC} Hapus Akun Kadaluarsa\n"
printf "                       ${GR}[${WH}12${GR}]${NC} Panel Bot Telegram\n"
printf "                       ${GR}[${WH}13${GR}]${NC} Backup & Restore\n"
printf "                       ${GR}[${WH}14${GR}]${NC} Menu Rebuild\n"

# Panel Admin (jika aktif)
if [ "$Isadmin" = "ON" ]; then
  printf "                       ${GR}[${WH}15${GR}]${NC} Menu Reseller IP\n"
  ressee="m-ip2"
  bottt="m-bot"
fi
echo -e "        ${GR}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
...
echo -e " ${WH}Aktif Hingga: ${YL}$Exp2${NC} (${WH}$(datediff "$Exp2" "$DATE")${NC}) | Credit: @AcilOffcial"
echo -e " ${WH}Ketik nomor atau ${GR}[0] / CTRL+C${NC} untuk keluar."
echo -e "${GR}══════════════════════════════════════════════════════════════════${NC}"
function new(){
