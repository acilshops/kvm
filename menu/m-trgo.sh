...
function addtrgo() {
  ...
  # === Panel Informasi Input (tanpa judul) ===
  echo -e "${COLOR1}╭═══════════════════════════════════════════════════╮${NC}"
  echo -e "${COLOR1}│${NC}  ${GREEN}Input ${WH}Username${NC}${GREEN}  ( example : Acilshop )             ${COLOR1}│${NC}"
  echo -e "${COLOR1}│${NC}  ${GREEN}Limit ${WH}Quota${NC}${GREEN}    ( example : 1000 )                 ${COLOR1}│${NC}"
  echo -e "${COLOR1}│${NC}  ${GREEN}Limit ${WH}IP${NC}${GREEN}       ( example : 100 )                  ${COLOR1}│${NC}"
  echo -e "${COLOR1}│${NC}  ${GREEN}Input 0 for ${WH}Quota Unlimited${NC}${GREEN} [GB]               ${COLOR1}│${NC}"
  echo -e "${COLOR1}│${NC}  ${GREEN}Input 0 for ${WH}Address Unlimited${NC}${GREEN} [IP]             ${COLOR1}│${NC}"
  echo -e "${COLOR1}│${NC}  ${GREEN}Input 0 for ${WH}Expired${NC}${GREEN} 360 [Days]                   ${COLOR1}│${NC}"
  echo -e "${COLOR1}╰═══════════════════════════════════════════════════╯${NC}"
  echo

  # === Prompt Username (validasi tetap sama) ===
  until [[ $user =~ ^[a-zA-Z0-9_]+$ && ${user_EXISTS} == '0' ]]; do
      echo -ne " ${WH}Input [Client] Username ( max 8 characters ) : ${NC}"
      read -r user
...
00 | 0) clear ; menu ;;
*) clear ; menu ;;
esacsed -i '/"'""$uuid""'"$/a\,"'""$user""'"' /etc/trojan-go/config.json
echo -e "### $user $exp" >> /etc/trojan-go/trgo
systemctl restart trojan-go.service
linktls="trojan-go://${user}@${domain}:2087/?sni=${domain}&type=ws&host=${domain}&path=/trojango&encryption=none#$user"
linktrgotls="trojan-go://${user}@${domain}:2087?%26sni=${domain}%26type=ws%26host=${domain}%26path=%2Ftrojango%26encryption=none#${user}"
